...
import { Search, AlertTriangle, TrendingUp, Package, BarChart3 } from "lucide-react";
...
export default function RiskAlertPage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}inventory/riskalert/`, {
                    headers: { Authorization: `Bearer ${token}` },
                });

                const items = res.data.risk_alerts || [];
                setData(items);
                ...
                const stockoutItems = items.filter(item => item.Live_Inventory <= item.Forecast_30).length;
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
            ...
            <div className="mb-8 flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 mb-2">Inventory Risk Alerts</h1>
                    <p className="text-gray-600">Monitor items at risk of stockouts</p>
                </div>
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-red-600">{stats.stockoutItems}</p>
                        </div>
                                    ...
                                    return (
                                        <React.Fragment key={item.SKU}>
                                            <tr className={`hover:bg-blue-50 transition-colors group ${isActive ? "bg-indigo-50" : ""}`}>
                                                ...
                                                <td className={`px-6 py-4 text-center ${item.Live_Inventory <= item.Forecast_30 ? "text-red-600" : "text-green-600"}`}>
                                                    {item.Live_Inventory}
                                                </td>
